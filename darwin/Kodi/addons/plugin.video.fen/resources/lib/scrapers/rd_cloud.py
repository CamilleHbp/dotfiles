...
class source:
	...
	def results(self, info):
		try:
			...
			from modules.kodi_utils import logger
			logger('FEN real-debrid scraper Exception', str(e))
	...
	def _scrape_cloud(self):
		try:
			...
			for item in my_cloud_files:
				...
				assigned_content = self._assigned_content(item['id'])
				if assigned_content: results_append((item['id'], assigned_content))
				elif not folder_name: results_append((item['id'], assigned_content))
					...
					results_append((item['id'], assigned_content))
